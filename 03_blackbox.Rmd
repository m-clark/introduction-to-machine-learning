...
In this example, we'll use the <span class="pack">glmnet</span> package, which will allow for a mix of lasso and ridge regression approaches.  Thus we'll have a penalty based on both absolute and squared values of the coefficients. In this case if we set the parameter to 1, we have your standard lasso penalty, and 0 would be the ridge penalty.  We have a another parameter, `lambda`, that would be the same as in the previous notation, i.e. the amount to penalize.

So what should `alpha` and `lambda` be set to?  They are examples of tuning parameters, for which we have no knowledge about its value without doing some initial digging.  As such we will select their values as part of the validation process. 
