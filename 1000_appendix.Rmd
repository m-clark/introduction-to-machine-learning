...
The following is a deep learning example using the data from the previous examples, using <span class="pack">keras</span>.  The package is an R wrapper for the <span class="pack">keras</span> module in Python, which itself is a wrapper for <span class="pack">tensorflow</span>.  
...
caret::confusionMatrix(y_test, ypred, positive='1')
```
...
Here is a bit of code to get you started with a sci-kit approach to deep learning. This is a 'deep neural net' with seven layers.  I also add some 'dropout', which, given the material presented, you can think of as a means of regularization.  This isn't too viable a model as far as settings go, and won't do any better than those shown previously[^dnn], but it'll run fine on your machine so you can at least play with it.
