...
```{r lvmodel, echo=FALSE, eval=T, dev='svg', background='#FFFFDD'}
library(DiagrammeR)
tags$div(style="width:25%; margin: 0 12.5%; bgcolor='transparent'",
DiagrammeR("
digraph DAG2 {
 ...
 LV1 [style=filled, fillcolor=lightblue, color=gray75,  fontcolor=gray50];
 LV2 [style=filled, fillcolor=lightblue, color=gray75,  fontcolor=gray50];
...
gnet = ggnetwork(n, weights='Weight', layout='kamadakawai')
ggsenate = ggplot(gnet, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_edges(aes(), color = "gray95", show.legend=F) + #size=Weight
  geom_nodes(aes(x, y, size=importance), color='transparent', show.legend=F) +
...
Example graph of the social network of senators based on data and filter at the following link. Node size is based on the *betweeness* centrality measure, edge size the percent agreement (graph filtered to edges >= 65%). Color is based on the clustering discovered within the graph [(link to data)](http://support.google.com/fusiontables/answer/2566732?hl=en&ref_topic=2572801)
...
- Compute the error rate $err_m = \frac{\overset{N}{\underset{i=1}{\sum}}\mathbb{I}(y_i\ne f^{(m)}_i)}{\overset{N}{\underset{i=1}{\sum}}w^{(m)}_i}$
...
Return $\textrm{sign} [\overset{M}{\underset{m=1}{\sum}}\alpha_m f^{(m)}]$
...
Aside from what has already been noted, there still exists a great many applications for ML such as data set shift[^datasetshift], deep learning[^deep], semi-supervised learning[^semi], online learning[^online], and many more.  
...
[^deep]: This includes learning at different levels of representation, e.g. from an image regarding a scene to the concepts used to describe it.
