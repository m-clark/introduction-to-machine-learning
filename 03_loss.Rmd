...
<span class="newthought">Given a set of predictor variables</span> $X$ and some target $y$, we look for some function $f(X)$ to make predictions of y from those input variables.  We also need a function to penalize errors in prediction- a <span class="emph">loss function</span>, $L(Y, f(X))$.  With a chosen loss function, we then find the model which will minimize loss, generally speaking.  We will start with the familiar and note a couple others that might be used.
